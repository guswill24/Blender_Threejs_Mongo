export default class LevelManager {
  ...
  goToLevel(levelNumber) {
    if (levelNumber < 1 || levelNumber > this.totalLevels) {
      console.warn(
        `⚠️ Nivel ${levelNumber} no válido. Debe estar entre 1 y ${this.totalLevels}`
      );
      return;
    }
  ...
  resetWorld() {
    ...
    if (world.loader) {
      ...
      console.log(
        `✅ Nivel actualizado en loader: ${world.loader.currentLevel}`
      );
    }
    ...
    if (world.enemies) {
      ...
      const baseEnemiesCount = parseInt(
        import.meta.VITE_ENEMIES_COUNT || "3",
        10
      );
      const enemiesCount = world._computeSpawnCountForLevel(
        this.currentLevel,
        baseEnemiesCount
      );
      setTimeout(() => {
    ...
    if (this.experience.menu?.setStatus) {
      this.experience.menu.setStatus(
        `🎮 Nivel ${this.currentLevel} - Puntos: 0`
      );
    }
  }
